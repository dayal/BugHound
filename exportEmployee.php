...
<?php
    ...
    $sel = mysql_query("SELECT * FROM employee");
    
    $filecsv = fopen('employee.csv', 'w');
    $filexml = fopen('employee.xml', 'w');
    $xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><employees>";
...
<script>
  $("form").submit(function(event) {
    ...
    if ($('#ascii').prop('checked')) {
        window.location.href = 'employee.csv';
    }
    if ($('#xml').prop('checked')) {
        window.location.href = 'employee.xml';
    }
