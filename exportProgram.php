...
<?php
    ...
    $sel = mysql_query("SELECT * FROM program");
    
    $filecsv = fopen('program.csv', 'w');
    $filexml = fopen('program.xml', 'w');
    $xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><programs>";
...
<script>
  $("form").submit(function(event) {
    ...
    if ($('#ascii').prop('checked')) {
        window.location.href = 'program.csv';
    }
    if ($('#xml').prop('checked')) {
        window.location.href = 'program.xml';
    }
