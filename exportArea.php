...
<?php
    ...
    $sel = mysql_query("SELECT * FROM area");
    
    $filecsv = fopen('area.csv', 'w');
    $filexml = fopen('area.xml', 'w');
    $xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><areas>";
...
<script>
  $("form").submit(function(event) {
    ...
    if ($('#ascii').prop('checked')) {
        window.location.href = 'area.csv';
    }
    if ($('#xml').prop('checked')) {
        window.location.href = 'area.xml';
    }
